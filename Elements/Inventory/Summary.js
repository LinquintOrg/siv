...
const Summary = React.forwardRef((props, ref) => {
	...
	const priceChange = (oldPrice, currentPrice) => {
		if (oldPrice > 0 && currentPrice > 0) {
			if (oldPrice > currentPrice) {
				return (
					<Text style={[styles.itemPriceChange, styles.pcDecrease,]}>
			...
			if (oldPrice < currentPrice) {
				return (
					<Text style={[styles.itemPriceChange, styles.pcIncrease,]}>
						+{ (((currentPrice - oldPrice) / oldPrice) * 100).toFixed(1) }%
					</Text>
				)
			}
		}
		return <Text style={[styles.itemPriceChange, styles.pcUnchanged,]}>0%</Text>
	}
	...
	const priceDiff = (oldPrice, currentPrice) => {
		...
		return (
			<Text bold style={[styles.itemPriceChange, (diff > 0 ? styles.pcIncrease : diff < 0 ? styles.pcDecrease : styles.pcUnchanged), bg]}>
				{ diff > 0 ? '+' : '' } { (Math.round(diff * rate * 100) / 100).toFixed(2) }
			</Text>
		);
	}
	...
	return (
		...
		>
			<BottomSheetScrollView contentContainerStyle={styles.scrollStyle}>
				...
				<View style={styles.dataRow}>
					<Text style={styles.statsTitle}>Total 24 hours ago</Text>
					<Text bold style={styles.statsValue}>{curr} {(Math.round(stats['p24ago'] * rate * 100) / 100).toFixed(2)}</Text>
				</View>
				<View style={styles.dataRow}>
					<Text style={styles.statsTitle}>Price change</Text>
					<View style={styles.row}>{ priceDiff(stats['p24ago'], stats['price']) }{ priceChange(stats['p24ago'], stats['price']) }</View>
				</View>

				<View style={styles.dataRow}>
					<Text style={styles.statsTitle}>Total 30 days ago</Text>
					<Text bold style={styles.statsValue}>{curr} {(Math.round(stats['p30ago'] * rate * 100) / 100).toFixed(2)}</Text>
				</View>
				<View style={styles.dataRow}>
					<Text style={styles.statsTitle}>Price change</Text>
					<View style={styles.row}>{ priceDiff(stats['p30ago'], stats['price']) }{ priceChange(stats['p30ago'], stats['price']) }</View>
				</View>

				<View style={styles.dataRow}>
					<Text style={styles.statsTitle}>Total 90 days ago</Text>
					<Text bold style={styles.statsValue}>{curr} {(Math.round(stats['p90ago'] * rate * 100) / 100).toFixed(2)}</Text>
				</View>
				<View style={styles.dataRow}>
					<Text style={styles.statsTitle}>Price change</Text>
					<View style={styles.row}>{ priceDiff(stats['p90ago'], stats['price']) }{ priceChange(stats['p90ago'], stats['price']) }</View>
				</View>
...

				{
					stats.games.map((data, index) => (
						<View>
							...
							<View style={styles.dataRow}>
								<Text style={styles.statsTitle}>Total 24 hours ago</Text>
								<Text bold style={styles.statsValue}>{curr} {(Math.round(data.p24ago * rate * 100) / 100).toFixed(2)}</Text>
							</View>
							<View style={styles.dataRow}>
								<Text style={styles.statsTitle}>Total 30 days ago</Text>
								<Text bold style={styles.statsValue}>{curr} {(Math.round(data.p30ago * rate * 100) / 100).toFixed(2)}</Text>
							</View>
							<View style={styles.dataRow}>
								<Text style={styles.statsTitle}>Total 90 days ago</Text>
								<Text bold style={styles.statsValue}>{curr} {(Math.round(data.p90ago * rate * 100) / 100).toFixed(2)}</Text>
							</View>
